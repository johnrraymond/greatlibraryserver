...
async function getBalance(_address) {
	const marketplace = new Contract(nft_market_place_abi, bookRegistryAccount);

	const nonceOperator = web3.eth.getTransactionCount(bookRegistryAccount);
	const functionCall = marketplace.methods.balanceOf(_address).encodeABI();
...
async function getpaid(event) {
        ...
        const options = {
                ...
                contractAddress: nft_market_place_address,
