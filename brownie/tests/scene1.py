...
FLAG_IS_NPC=10000
ARCANE_ORB=1
def scene_one(heros, timecube, loot, slot):
    ...
    ## DAEDALUS resurects GAZ.
    timecube.castRES(DAEDALUS, GAZ, {'from': accounts[0].address})
    LOG.info("Daedalus resurects Gaz.")

    # transmute(uint256 _hId, int _slot, uint _time, uint _what, uint _amount)
    ## Rennly transmutes the loot off Gaz in the cube
    item = timecube.transmute(RENNLY, slot, 1, GAZ, 1, {'from': accounts[0].address})
    LOG.info("Rennly transmutes the loot he got off Gaz in the cube." + str(item.events))
    LOG.info("New loot: " + str(item.events["Transmute"]["newItem"]))

    itemId = item.events["Transmute"]["newItem"]
    
    ## Rennly equipts the new item.
    timecube.equipItem(RENNLY, itemId, {'from': accounts[0].address})

    ##cubeItem(uint256 _hId, uint256 _iId, int _cubeSlot)
    #timecube.cubeItem(uint256 _hId, uint256 _iId, int _cubeSlot)

def test_scene_one_reverts(heros, timecube, loot):
    scene_one(heros, timecube, loot, 1)
    scene_one(heros, timecube, loot, 2)
    scene_one(heros, timecube, loot, 3)
    scene_one(heros, timecube, loot, 4)
    scene_one(heros, timecube, loot, 5)
    scene_one(heros, timecube, loot, 6)
    scene_one(heros, timecube, loot, 7)
    scene_one(heros, timecube, loot, 8)
